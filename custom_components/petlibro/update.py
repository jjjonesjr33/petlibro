...
class PetLibroUpdateEntityDescription(
    UpdateEntityDescription, PetLibroEntityDescription[_DeviceT]
):
...
class PetLibroUpdateEntity(PetLibroEntity[_DeviceT], UpdateEntity):
    ...
    def __init__(
        ...
        device: _DeviceT,
        hub: PetLibroHub,
        description: PetLibroUpdateEntityDescription[_DeviceT],
    ) -> None:
    ...
    def installed_version(self) -> str:
        ...
        version = (
            ...
            or self._attr_installed_version
        )
    ...
    def display_precision(self) -> int:
        ...
        _LOGGER.debug(
            "[UpdateEntity] display_precision returning: %s",
            self._attr_display_precision,
        )
    ...
    def in_progress(self) -> bool:
        ...
        _LOGGER.debug(
            "[UpdateEntity] in_progress returning: %s (progress=%s)",
            in_progress,
            progress,
        )
        return in_progress
    ...
    def update_percentage(self) -> float | None:
        ...
        value = (
            float(progress)
            if progress is not None and 0.0 < progress <= 100.0
            else None
        )
        _LOGGER.debug(
            "[UpdateEntity] update_percentage returning: %s (raw progress=%s)",
            value,
            progress,
        )
        return value
    ...
    async def async_install(
        ...
        """Install an update."""
        _LOGGER.debug(
            "Install called with version=%s backup=%s kwargs=%s",
            version,
            backup,
            kwargs,
        )
        ...
        _LOGGER.debug(
            "Triggering firmware update for %s (jobItemId=%s)",
            self.device.name,
            job_item_id,
        )
...
async def async_setup_entry(
    ...
    if not entities:
        ...
        for entity in entities:
            _LOGGER.debug(
                "Adding update entity: %s for device %s",
