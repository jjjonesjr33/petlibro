...
class PetLibroSession:
    ...
    async def get(self, path: str, params: dict | None = None, **kwargs: Any) -> JSON:
    ...
    async def request(self, method: str, url: str, **kwargs: Any) -> JSON:
        ...
        async with self.websession.request(method, joined_url, **kwargs) as resp:
            ...
            if data.get("code") != 0:
                msg = f"Code: {data.get('code')}, Message: {data.get('msg')}"
                raise PetLibroAPIError(msg)
    ...
    async def re_login(self) -> str:
        ...
        try:
            _LOGGER.debug(
                "Attempting re-login with email: %s and region: %s",
                self.email,
                self.region,
            )
            ...
            async with self.websession.post(
                ...
                if response.status != 200:
                    msg = f"Failed to login, status: {response.status}"
                    raise PetLibroAPIError(msg)
                ...
                if not isinstance(
                    ...
                    msg = "Token not found during login."
                    raise PetLibroAPIError(msg)
                ...
                if hasattr(self, "api") and self.api.hass and self.api.config_entry:
                    _LOGGER.debug("Saving new token to config entry: %s", self.token)
            ...
            _LOGGER.exception("Re-login failed due to a client error")
            msg = f"Client error during re-login: {e}"
            raise PetLibroAPIError(msg) from e

        except Exception as e:
            _LOGGER.exception("Re-login attempt failed due to an unexpected error")
            msg = f"Unexpected error during re-login: {e}"
            raise PetLibroAPIError(msg) from e
...
class PetLibroAPI:
    ...
    def __init__(
        ...
        self.session = PetLibroSession(
            API_URLS[region], session, email, password, region, token, time_zone
        )
        self.region = region
        ...
        if config_entry and "token" in config_entry.data:
            ...
            _LOGGER.debug("Loaded saved token: %s", self.token)
    ...
    def hash_password(password: str) -> str:
        ...
        return md5(password.encode("UTF-8")).hexdigest()  # noqa: S324
    ...
    async def login(self, email: str, password: str) -> str:
        ...
        try:
            ...
            if (
                ...
                msg = "No token found during login."
                raise PetLibroAPIError(msg)

            self.session.token = data["token"]
            _LOGGER.debug("Login successful, token: %s", self.session.token)

        except Exception as e:
            _LOGGER.exception("Login failed")
            msg = f"Login attempt failed: {e}"
            raise PetLibroAPIError(msg) from e
    ...
    async def get_device_real_info(self, device_id: str) -> dict:
        ...
        if last_call_time and (now - last_call_time) < timedelta(seconds=10):
            _LOGGER.debug(
                "Skipping realInfo request for %s, using cached response.", device_id
            )
        ...
        try:
            ...
            _LOGGER.exception("Error fetching realInfo for device %s", device_id)
            msg = f"Error fetching realInfo for device {device_id}: {e}"
            raise PetLibroAPIError(msg) from e
        else:
            return response
    ...
    async def get_device_data_real_info(self, device_id: str) -> dict:
        ...
        if last_call_time and (now - last_call_time) < timedelta(seconds=10):
            _LOGGER.debug(
                "Skipping dataRealInfo request for %s, using cached response.",
                device_id,
            )
        ...
        try:
            ...
            _LOGGER.exception("Error fetching _dataRealInfo for device %s", device_id)
            msg = f"Error fetching _dataRealInfo for device {device_id}: {e}"
            raise PetLibroAPIError(msg) from e
        else:
            return response
    ...
    async def get_device_drink_water(self, device_id: str) -> dict:
        ...
        if last_call_time and (now - last_call_time) < timedelta(seconds=10):
            _LOGGER.debug(
                "Skipping drinkWater request for %s, using cached response.", device_id
            )
        ...
        try:
            ...
            _LOGGER.exception("Error fetching _drinkWater for device %s", device_id)
            msg = f"Error fetching _drinkWater for device {device_id}: {e}"
            raise PetLibroAPIError(msg) from e
        else:
            return response
    ...
    async def get_device_attribute_settings(self, device_id: str) -> dict:
        ...
        if last_call_time and (now - last_call_time) < timedelta(seconds=10):
            _LOGGER.debug(
                "Skipping getAttributeSetting request for %s, using cached response.",
                device_id,
            )
        ...
        try:
            ...
            _LOGGER.exception(
                "Error fetching getAttributeSetting for device %s", device_id
            )
            msg = f"Error fetching getAttributeSetting for device {device_id}: {e}"
            raise PetLibroAPIError(msg) from e
        else:
            return response
    ...
    async def get_device_upgrade(self, device_id: str) -> dict:
        ...
        if last_call_time and (now - last_call_time) < timedelta(seconds=10):
            _LOGGER.debug(
                "Skipping getUpgrade request for %s, using cached response.", device_id
            )
        ...
        try:
            ...
            _LOGGER.exception("Error fetching getUpgrade for device %s", device_id)
            msg = f"Error fetching getUpgrade for device {device_id}: {e}"
            raise PetLibroAPIError(msg) from e
        else:
            return response
    ...
    async def get_device_base_info(self, device_id: str) -> dict:
        ...
        if last_call_time and (now - last_call_time) < timedelta(seconds=10):
            _LOGGER.debug(
                "Skipping baseInfo request for %s, using cached response.", device_id
            )
        ...
        try:
            ...
            _LOGGER.exception("Error fetching baseInfo for device %s", device_id)
            msg = f"Error fetching baseInfo for device {device_id}: {e}"
            raise PetLibroAPIError(msg) from e
        else:
            return response
    ...
    async def get_device_work_record(self, device_id: str) -> dict:
        ...
        if last_call_time and (now - last_call_time) < timedelta(seconds=10):
            _LOGGER.debug(
                "Skipping workRecord request for %s, using cached response.", device_id
            )
        ...
        try:
            ...
            _LOGGER.exception("Error fetching workRecord for device %s", device_id)
            msg = f"Error fetching workRecord for device {device_id}: {e}"
            raise PetLibroAPIError(msg) from e
    ...
    async def get_device_events(self, device_id: str) -> dict:
        ...
        if last_call_time and (now - last_call_time) < timedelta(seconds=10):
            _LOGGER.debug(
                "Skipping deviceEvents request for %s, using cached response.",
                device_id,
            )
        ...
        try:
            ...
            _LOGGER.exception("Error fetching deviceEvents for device %s", device_id)
            msg = f"Error fetching deviceEvents for device {device_id}: {e}"
            raise PetLibroAPIError(msg) from e
        else:
            return response
    ...
    async def get_default_matrix(self, device_sn: str) -> dict:
        ...
        if last_call_time and (now - last_call_time) < timedelta(seconds=10):
            _LOGGER.debug("Using cached response for getDefaultMatrix: %s", device_sn)
        ...
        try:
            ...
            _LOGGER.exception("Error fetching default matrix for device %s", device_sn)
            msg = f"Failed to fetch default matrix: {e}"
            raise PetLibroAPIError(msg) from e
        else:
            return response
    ...
    async def device_attribute_settings(self, serial: str) -> dict[str, Any]:
        """Retrieve device attribute settings."""
        return await self.session.post_serial(
    ...
    async def device_feeding_plan_list(self, serial: str) -> list[dict[str, Any]]:
        """Retrieve device feeding plan list."""
    ...
    async def device_wet_feeding_plan(self, serial: str) -> dict[str, Any]:
        """Retrieve device wet feeding plan."""
        return await self.session.post_serial(
            "/device/wetFeedingPlan/wetListV3", serial
        )

    # Support for new switch functions
    async def set_feeding_plan(self, serial: str, enable: bool) -> None:
    ...
    async def set_child_lock(self, serial: str, enable: bool) -> None:
        ...
        try:
            ...
            _LOGGER.debug("Child lock response status: %s", response.status)
            _LOGGER.debug("Child lock response data: %s", await response.text())

            response.raise_for_status()
        except aiohttp.ClientError as err:
            _LOGGER.exception("Failed to set child lock for device %s", serial)
            msg = f"Error setting child lock: {err}"
            raise PetLibroAPIError(msg) from err

    async def set_light_enable(self, serial: str, enable: bool) -> None:
        ...
        try:
            ...
            _LOGGER.exception("Failed to set light enable for device %s", serial)
            msg = f"Error setting light enable: {err}"
            raise PetLibroAPIError(msg) from err

    async def set_light_switch(self, serial: str, enable: bool) -> None:
    ...
    async def set_sound_enable(self, serial: str, enable: bool) -> None:
        ...
        try:
            ...
            _LOGGER.exception("Failed to set sound enable for device %s", serial)
    ...
    async def set_desiccant_cycle(self, serial: str, value: float, key: str) -> JSON:
        ...
        _LOGGER.debug(
            "Setting desiccant cycle: serial=%s, value=%s, key=%s", serial, value, key
        )
        try:
            ...
            _LOGGER.exception("Failed to set desiccant cycle for device %s", serial)
            raise
        else:
            return response

    async def set_sound_switch(self, serial: str, enable: bool) -> None:
    ...
    async def set_sound_level(self, serial: str, value: float) -> None:
        ...
        try:
            ...
            _LOGGER.exception("Failed to set sound level for device %s", serial)
            raise
        else:
            return response

    async def set_lid_close_time(self, serial: str, value: float) -> None:
        ...
        try:
            ...
            _LOGGER.exception("Failed to set lid close time for device %s", serial)
            raise
        else:
            return response

    async def set_lid_speed(self, serial: str, value: str) -> None:
        """Set the lid speed."""
        ...
        try:
            ...
            _LOGGER.exception("Failed to set lid speed for device %s", serial)
            raise
        else:
            return response

    async def set_vacuum_mode(self, serial: str, value: str) -> None:
        """Set the vacuum mode."""
        ...
        try:
            ...
            _LOGGER.exception(
                "Failed to set water dispensing mode for device %s", serial
            )
            raise
        else:
            return response
    ...
    async def set_water_low_threshold(self, serial: str, value: float) -> None:
        ...
        try:
            ...
            _LOGGER.exception("Failed to set water low threshold for device %s", serial)
            raise
        else:
            return response
    ...
    async def set_water_interval(
        ...
        try:
            ...
            _LOGGER.exception("Failed to set water interval for device %s", serial)
            raise
        else:
            return response
    ...
    async def set_water_dispensing_duration(
        ...
        """Set the water interval."""
        _LOGGER.debug(
            "Setting water dispensing duration: serial=%s, value=%s", serial, value
        )
        try:
            ...
            _LOGGER.exception(
                "Failed to set water dispensing duration for device %s", serial
            )
            raise
        else:
            return response
    ...
    async def set_cleaning_cycle(self, serial: str, value: float, key: str) -> JSON:
        ...
        _LOGGER.debug(
            "Setting machine cleaning cycle: serial=%s, value=%s, key=%s",
            serial,
            value,
            key,
        )
        try:
            ...
            _LOGGER.exception(
                "Failed to set machine cleaning cycle for device %s", serial
            )
            raise
        else:
            return response
    ...
    async def set_filter_cycle(self, serial: str, value: float, key: str) -> JSON:
        ...
        _LOGGER.debug(
            "Setting filter cycle: serial=%s, value=%s, key=%s", serial, value, key
        )
        try:
            ...
            _LOGGER.exception("Failed to set filter cycle for device %s", serial)
            raise
        else:
            return response

    async def set_lid_mode(self, serial: str, value: str) -> JSON:
        """Set the lid mode."""
        ...
        try:
            ...
            _LOGGER.exception("Failed to set lid mode for device %s", serial)
            raise
        else:
            return response

    async def set_water_mode_off(self, serial: str) -> JSON:
        ...
        try:
            ...
            _LOGGER.exception("Failed to set OFF for %s", serial)
            raise
        else:
            return resp

    async def set_water_mode_on(self, serial: str) -> JSON:
        ...
        try:
            ...
            _LOGGER.exception("Failed to set ON for %s", serial)
            raise
        else:
            return resp
    ...
    async def set_water_mode_radar_near(
        ...
        *,
        ...
        try:
            ...
            _LOGGER.exception("Failed to set Sensed Near for %s", serial)
            raise
        else:
            return mode_resp
    ...
    async def set_water_mode_radar_far(
        ...
        *,
        ...
        try:
            ...
            _LOGGER.exception("Failed to set Sensed Far for %s", serial)
            raise
        else:
            return mode_resp
    ...
    async def set_new_water_mode_intermittent(
        ...
        *,
        ...
        """
        Set new water intermittent mode.
        ...
        try:
            ...
            _LOGGER.exception("Failed to set Intermittent for %s", serial)
            raise
        else:
            return resp
    ...
    async def set_new_water_mode_constant(
        ...
        *,
        ...
        try:
            ...
            _LOGGER.exception("Failed to set Constant for %s", serial)
            raise
        else:
            return resp
    ...
    async def set_display_icon(self, serial: str, value: float) -> JSON:
        ...
        try:
            ...
            _LOGGER.exception("Failed to set display icon for device %s", serial)
            raise
        else:
            return response

    async def set_display_text(self, serial: str, value: str) -> JSON:
        ...
        try:
            ...
            _LOGGER.exception("Failed to set display text for device %s", serial)
            raise
        else:
            return response

    async def set_manual_feed(
        self, serial: str, feed_value: int = 1
    ) -> JSON:  # Provide a default argument for the feed value just in case this works differently with other feeders
        ...
        try:
            ...
            if response.status != 200 or response_data.get("code") != 0:
                msg = f"Failed to trigger manual feeding: {response_data.get('msg')}"
                raise PetLibroAPIError(msg)

        except aiohttp.ClientError as err:
            _LOGGER.exception("Failed to trigger manual feeding for device %s", serial)
            msg = f"Error triggering manual feeding: {err}"
            raise PetLibroAPIError(msg) from err

        else:
            return response_data

    async def set_manual_feed_now(self, serial: str, plate: int) -> None:
        ...
        try:
            ...
            _LOGGER.exception("Failed to trigger manual feed now for device %s", serial)
            msg = f"Error triggering manual feed now: {err}"
            raise PetLibroAPIError(msg) from err

    async def set_stop_feed_now(self, serial: str, manual_feed_id: int) -> None:
        ...
        try:
            ...
            _LOGGER.exception("Failed to trigger stop feed now for device %s", serial)
            msg = f"Error triggering stop feed now: {err}"
            raise PetLibroAPIError(msg) from err
    ...
    async def set_rotate_food_bowl(self, serial: str) -> int:
        ...
        try:
            ...
            _LOGGER.debug(
                "Rotate food bowl successful, new plate position: %s", response
            )

        except aiohttp.ClientError as err:
            _LOGGER.exception(
                "Failed to trigger rotate food bowl for device %s", serial
            )
    ...
    async def set_feed_audio(self, serial: str) -> None:
        ...
        try:
            ...
            _LOGGER.exception("Failed to trigger feed audio for device %s", serial)
    ...
    async def set_desiccant_reset(self, serial: str) -> JSON:
        ...
        try:
            ...
            if isinstance(response, int):
                _LOGGER.debug(
                    "Desiccant reset set successfully, returned code: %s", response
                )
                return response

            # If response is a dictionary (JSON), handle it
            response_data = await response.json()
            _LOGGER.debug("Desiccant reset response data: %s", response_data)

            # Check if the response indicates success
            if response.status != 200 or response_data.get("code") != 0:
                msg = "Failed to trigger desiccant reset: %s"
                raise PetLibroAPIError(msg, response_data.get("msg"))

        except aiohttp.ClientError as err:
            _LOGGER.exception("Failed to trigger desiccant reset for device %s", serial)
            msg = f"Error triggering desiccant reset: {err}"
            raise PetLibroAPIError(msg) from err

        else:
            return response_data

    async def trigger_firmware_upgrade(self, serial: str, job_item_id: str) -> JSON:
        """Trigger the firmware upgrade for the device."""
        _LOGGER.debug(
            "Triggering firmware upgrade: serial=%s, jobItemId=%s", serial, job_item_id
        )
        try:
            response = await self.session.post(
            ...
            _LOGGER.exception(
                "Failed to trigger firmware upgrade for device %s", serial
            )
            raise
        else:
            return response
    ...
    async def set_cleaning_reset(self, serial: str) -> JSON:
        ...
        _LOGGER.debug(
            "Triggering machine cleaning reset for device with serial: %s", serial
        )
        ...
        try:
            ...
            if isinstance(response, int):
                _LOGGER.debug(
                    "Machine cleaning reset set successfully, returned code: %s",
                    response,
                )
                return response
            ...
            # Check if the response indicates success
            if response.status != 200 or response_data.get("code") != 0:
                msg = f"Failed to trigger machine cleaning reset: {response_data.get('msg')}"
                raise PetLibroAPIError(msg)

        except aiohttp.ClientError as err:
            _LOGGER.exception(
                "Failed to trigger machine cleaning reset for device %s", serial
            )
    ...
    async def set_filter_reset(self, serial: str) -> JSON:
        ...
        try:
            ...
            if isinstance(response, int):
                _LOGGER.debug(
                    "Filter reset set successfully, returned code: %s", response
                )
                return response
    ...
    async def set_manual_lid_open(self, serial: str) -> None:
    ...
    async def set_display_on(self, serial: str) -> None:
    ...
    async def set_display_off(self, serial: str) -> None:
    ...
    async def set_light_on(self, serial: str) -> None:
    ...
    async def set_light_off(self, serial: str) -> None:
    ...
    async def set_sound_on(self, serial: str) -> None:
    ...
    async def set_sound_off(self, serial: str) -> None:
    ...
    async def set_sleep_on(self, serial: str) -> None:
    ...
    async def set_sleep_off(self, serial: str) -> None:
    ...
    async def set_reposition_schedule(
        ...
        _LOGGER.debug(
            "Triggering reposition schedule for device with serial: %s", serial
        )
        await self.session.post(
    ...
    async def member_info(self) -> dict[str, Any]:
        ...
        try:
            ...
            msg = "Failed to fetch member information"
            raise PetLibroAPIError(msg) from exc

        if not isinstance(data, dict):
            msg = f"Invalid member info response format: {data}"
            raise PetLibroAPIError(msg)
    ...
    async def member_update_info(
        ...
        async def _post_update(endpoint: str, payload: dict[str, Any]) -> bool:
            """Send settings and handle logging."""
            try:
