...
class OneRFIDSmartFeeder(DryFoodFeeder):
    """Petlibro One RFID Smart Feeder."""
    ...
    async def refresh(self) -> None:
        ...
        try:
            ...
            _LOGGER.exception("Error refreshing data for OneRFIDSmartFeeder")
    ...
    def display_switch(self) -> bool:
        """Return the display switch status."""
            ...
            for day_entry in raw:
                for record in day_entry.get("workRecords", []):
                    ...
                    if record.get("type") == "GRAIN_OUTPUT_SUCCESS":
                        ...
                        return (
                            actual_grain_num if isinstance(actual_grain_num, int) else 0
                        )
        return 0
    ...
    async def set_desiccant_cycle(self, value: float) -> None:
        """Set desiccant cycle."""
        _LOGGER.debug("Setting desiccant cycle to %s for %s", value, self.serial)
        try:
            key = "DESSICANT"
            await self.api.set_desiccant_cycle(self.serial, value, key)
            await self.refresh()  # Refresh the state after the action
        except aiohttp.ClientError as err:
            _LOGGER.exception("Failed to set desiccant cycle for %s", self.serial)
            msg = f"Error setting desiccant cycle: {err}"
    ...
    async def set_manual_lid_open(self) -> None:
        """Set manual lid open."""
        _LOGGER.debug("Triggering manual lid opening for %s", self.serial)
        try:
            await self.api.set_manual_lid_open(self.serial)
            await self.refresh()  # Refresh the state after the action
        except aiohttp.ClientError as err:
            _LOGGER.exception(
                "Failed to trigger manual lid opening for %s", self.serial
            )
            msg = f"Error triggering manual lid opening: {err}"
            raise PetLibroAPIError(msg) from err

    # Method for display turn on
    async def set_display_on(self) -> None:
        """Turn on the display."""
        _LOGGER.debug("Turning on the display matrix for %s", self.serial)
        try:
            await self.api.set_display_on(self.serial)
            await self.refresh()  # Refresh the state after the action
        except aiohttp.ClientError as err:
            _LOGGER.exception("Failed to turn on the display for %s", self.serial)
            msg = f"Error turning on the display: {err}"
            raise PetLibroAPIError(msg) from err

    # Method for display matrix turn off
    async def set_display_off(self) -> None:
        """Turn off the display."""
        _LOGGER.debug("Turning off the display for %s", self.serial)
        try:
            await self.api.set_display_off(self.serial)
            await self.refresh()  # Refresh the state after the action
        except aiohttp.ClientError as err:
            _LOGGER.exception("Failed to turn off the display for %s", self.serial)
            msg = f"Error turning off the display: {err}"
            raise PetLibroAPIError(msg) from err
    ...
    async def set_desiccant_reset(self) -> None:
        """Set desiccant reset."""
        _LOGGER.debug("Triggering desiccant reset for %s", self.serial)
        try:
            await self.api.set_desiccant_reset(self.serial)
            await self.refresh()  # Refresh the state after the action
        except aiohttp.ClientError as err:
            _LOGGER.exception("Failed to trigger desiccant reset for %s", self.serial)
    ...
    def lid_speed(self) -> str:
        ...
        api_value = self._data.get("getAttributeSetting", {}).get(
    ...
    async def set_lid_speed(self, value: str) -> None:
        """Set lid speed."""
        _LOGGER.debug("Setting lid speed to %s for %s", value, self.serial)
        try:
            await self.api.set_lid_speed(self.serial, value)
            await self.refresh()  # Refresh the state after the action
        except aiohttp.ClientError as err:
            _LOGGER.exception("Failed to set lid speed for %s", self.serial)
            msg = f"Error setting lid speed: {err}"
            raise PetLibroAPIError(msg) from err
    ...
    async def set_lid_mode(self, value: str) -> None:
        """Set lid mode."""
        _LOGGER.debug("Setting lid mode to %s for %s", value, self.serial)
        try:
            await self.api.set_lid_mode(self.serial, value)
            await self.refresh()  # Refresh the state after the action
        except aiohttp.ClientError as err:
            _LOGGER.exception("Failed to set lid mode for %s", self.serial)
            msg = f"Error setting lid mode: {err}"
            raise PetLibroAPIError(msg) from err

    @property
    def lid_close_time(self) -> float:
        """Return the lid close time."""
    ...
    async def set_lid_close_time(self, value: float) -> None:
        ...
        _LOGGER.debug("Setting lid close time to %s for %s", value, self.serial)
        try:
            await self.api.set_lid_close_time(self.serial, value)
            await self.refresh()  # Refresh the state after the action
        except aiohttp.ClientError as err:
            _LOGGER.exception("Failed to set lid close time for %s", self.serial)
            msg = f"Error setting lid close time: {err}"
            raise PetLibroAPIError(msg) from err
    ...
    async def set_display_text(self, value: str) -> None:
        ...
        _LOGGER.debug("Setting display text to %s for %s", value, self.serial)
        try:
            await self.api.set_display_text(self.serial, value)
            await self.refresh()  # Refresh the state after the action
        except aiohttp.ClientError as err:
            _LOGGER.exception("Failed to set display text for %s", self.serial)
            msg = f"Error setting display text: {err}"
            raise PetLibroAPIError(msg) from err
    ...
    async def set_display_icon(self, value: float) -> None:
        """Set display icon."""
        _LOGGER.debug("Setting display icon to %s for %s", value, self.serial)
        try:
            await self.api.set_display_icon(self.serial, value)
            await self.refresh()  # Refresh the state after the action
        except aiohttp.ClientError as err:
            _LOGGER.exception("Failed to set display icon for %s", self.serial)
            msg = f"Error setting display icon: {err}"
            raise PetLibroAPIError(msg) from err

    @property
    def display_selection(self) -> str:
        """Return the display selection."""
        display_text = self._data.get("getDefaultMatrix", {}).get("screenLetter", None)
        display_icon = self._data.get("getDefaultMatrix", {}).get(
